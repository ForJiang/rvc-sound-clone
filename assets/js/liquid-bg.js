...
/**
 * 鼠标白色流光轨迹（对齐个人站的 MouseTrail：纯白、lighter 混合）。
 * @returns {{stop: () => void}}
 */
export function startMouseTrail(canvas) {
  ...
  function frame() {
    ...
    if (visible && points.length > 1) {
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        const fade = i / points.length;                 // 头部亮、尾部渐隐
        // 外层柔光
        ctx.strokeStyle = `rgba(255,255,255,${0.16 * fade})`;
        ctx.lineWidth = 9 * dpr * fade;
        ctx.beginPath();
      ...
      const g = ctx.createRadialGradient(head.x, head.y, 0, head.x, head.y, 26 * dpr);
      g.addColorStop(0, 'rgba(255,255,255,0.20)');
      g.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.fillStyle = g;
      ctx.beginPath();
      ctx.arc(head.x, head.y, 26 * dpr, 0, Math.PI * 2);
      ctx.fill();

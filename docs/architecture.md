...
- **电平波形、电平条、电平画布共用同一条 rAF 循环**（都在 `Recorder.start()` 的 loop 里）。
  曾经分两条循环跑：每帧要把 analyser 读两遍、画布画两遍，而且 `attachMeter()` 每次重渲染
  都会再起一条，切 10 次 tab 实测多攒 5 条。现在 `attachMeter()` 退化成只登记画布。
  `ui.js` 的 `drawMeter()` 另有一层优化：渐变按条高缓存（一帧本来要建 24 个
  `LinearGradient`），高度随布局变化时清缓存——渐变坐标空间随高度变，不能跨高度复用。
...
内容统一压在玻璃面板上保证可读性。背景渲染带画质自适应档位（3840×2160 起步，p95 帧时间超标才逐级降档，
最低 1600×900），`prefers-reduced-motion` 下静态成一帧，WebGL 不可用时回落到 CSS 渐变。

清晰度上有一条硬约束：波形按归一化坐标生成，与渲染分辨率无关，**多给像素只会让细亮线更锐、不改变画面**。
所以画布要按屏幕实拍密度渲染（`DPR_CAP = 2`）——低于 `devicePixelRatio` 时浏览器会把画布放大，
细线立刻发虚（实测 DPR2 的屏上只渲染显示像素的 75%）。档位上限同理，宁可留给自适应去降，
也不要一开始就欠采样。尺寸另有一道 GL 上限兜底（`MAX_VIEWPORT_DIMS`）：超了 `drawArrays`
会静默失败、画面直接全黑。画布尺寸由 `ResizeObserver` + `resize` 事件驱动（不在每帧里读
`clientWidth`——那等于每帧强制一次同步 layout），DPR 变化时 CSS 尺寸不变、ResizeObserver
不触发，所以 `resize` 兜底必须留着。
